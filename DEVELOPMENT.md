...
本压缩包用于 RecoverHand Windows 上位机的界面、流程、模拟设备、三维粒子手和独立肌电监视器开发。主上位机仍是研发模拟原型；只有操作者单独启动肌电监视器时才连接唯理贴片，而且监视器不保存数据、不发送控制命令，不能用于无人监督训练或评价康复效果。
...
```text
backend/recoverhand_host/   Python 业务核心、设备抽象与 Qt 桌面界面
